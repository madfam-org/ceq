...
- **Monetization gating**: `apps/api/src/ceq_api/routers/interest.py` + `apps/studio/src/components/InterestGate.tsx`. Premium-tagged templates (`Template.tags` includes 'pro' or 'premium') show overlay InterestGate for free users; data captured in `feature_interest` table → CRM webhook → Phynd-CRM. Switches to checkout when `recommended_action` from Tulana flips.
