...
> **Creative Entropy Quantized — ComfyUI wrapper + /v1/render asset pillar for MADFAM.**
...
**Registry product**: CEQ (`ceq`, degraded, front door ceq.lol).
...
The commands below default to `ceq-api` — the primary service name for
...
# Logs
enclii logs ceq-api --env prod -f                    # live tail
enclii logs ceq-api --env prod --since 1h -n 200     # last hour
...
# Rollback
enclii rollback ceq-api --env prod                   # previous release
enclii rollback ceq-api v42 --env prod

# Releases + deployment history
enclii releases ceq-api -n 20
...
enclii domains list --service ceq-api
enclii domains add my.example.com --service ceq-api   # auto-provisions tunnel route + DNS
